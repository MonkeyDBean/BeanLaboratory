...
//        list3.addAll(list4);
     ...
     * 测试long转int, 转换后可能为负数
     * <p>
     * short为16位, 范围为-32768～32767
     * int为32位, 范围为 -2147483648～2147483647
     * long为64位, 范围为 -9223372036854775808～9223372036854775807
     * <p>
     * 注意延时队列的compareTo方法, 强转后可能为负数, 导致排在后面的单元数据先被执行
     * 实际一般不会遇到此类问题, 因为延迟执行时间太长(2147483647 / 1000 /3600 / 24 ≈ 25天)
     * 若有此类需要, 也不采用延时队列的方式而是采用定时脚本跑持久化数据等方式
     */
    @Test
    public void testLongToInt() {

        //对应二进制格式为0000000000000000000000000000000010000000000000000000000000000000
        long originValue = 2147483648L;

        //强转为int, 截取后32位(其中左边第一位为符号, 后31位为数值位)为-2147483648(负数以补码的方式存储)
        int convertValue = (int) originValue;
